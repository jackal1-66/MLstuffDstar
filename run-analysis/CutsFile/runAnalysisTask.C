...
void runAnalysis()
{
    ...
    AliAnalysisTaskSEDStarSpectra *taskDstar = reinterpret_cast<AliAnalysisTaskSEDStarSpectra *>(gInterpreter->ProcessLine(Form(".x %s (%d, %d, %d, \"%s\", \"%s\")", gSystem->ExpandPathName("$ALICE_PHYSICS/PWGHF/vertexingHF/macros/AddTaskDStarSpectra.C"),0,0,100,"DStartoKpipiCuts_stringfix.root","mgiacaloDstar")));

    AliAnalysisTaskSEDvsMultiplicity *taskDvsMult = reinterpret_cast<AliAnalysisTaskSEDvsMultiplicity*>(gInterpreter->ProcessLine(Form(".x %s (%d, %d, %d, %d, \"%s\", \"%s\", \"%s\", \"%s\", %f, %d, %d, %d, %d, %d, %d, %d)", gSystem->ExpandPathName("$ALICE_PHYSICS/PWGHF/vertexingHF/macros/AddTaskDvsMultiplicity.C"),0,kFALSE,0,413,"mgiacalo","DStartoKpipiCuts_stringfix.root","DStartoKpipiCuts","alien:///alice/cern.ch/user/c/cterrevo/ntrackl18.root",12.25,kTRUE,kFALSE,AliAnalysisTaskSEDvsMultiplicity::kNtrk10,AliAnalysisTaskSEDvsMultiplicity::kEta10,kFALSE,18, kTRUE)));
...
//    task0->SelectCollisionCandidates(AliVEvent::kINT7);
    ...
    if(local) {
       ...
       alienHandler->SetAliPhysicsVersion("vAN-20190623_ROOT6-1");
        ...
        alienHandler->SetExecutable("myTask_testDst_290719.sh");
        // specify how many seconds your job may take
        alienHandler->SetTTL(70000);
        alienHandler->SetJDLName("myTask_testDst_290719.jdl");
        ...
        alienHandler->SetGridWorkingDir("testDst_290719");
